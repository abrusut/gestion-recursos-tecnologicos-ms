...
use App\Entity\SituacionRevista;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Doctrine\Common\Collections\Criteria;
use Doctrine\ORM\Tools\Pagination\Paginator as DoctrinePaginator;
use Symfony\Bridge\Doctrine\RegistryInterface;

class SituacionRevistaRepository extends ServiceEntityRepository
{
    ...
    {
        parent::__construct($registry, SituacionRevista::class);
    }
    ...
    public function findByTermino($termino, $page = 1, $size = 20, $order)
    {
        ...
        $queryBuilder = $this->createQueryBuilder('st')
            ->andWhere('st.tipo LIKE :termino')
            ->setParameter('termino', '%'.$termino.'%');
        
        if(!is_null($order) && count($order)>0){
            foreach ($order as  $clave => $valor)
            {
                $queryBuilder->orderBy('st.'.$clave , $valor);
            }
        }
